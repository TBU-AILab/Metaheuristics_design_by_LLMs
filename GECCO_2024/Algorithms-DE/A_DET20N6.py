...
class Algorithm():
    def __init__(self, func, dim, bounds, max_evals, params=None):
        if params is None:
            self.params = {
                ...
                'epsilon': 1e-10,
                'learning_period': 50,
                'mutation_strategy': 'current-to-best/2/bin',  # Updated mutation strategy
                'adaptive_CR': True,  # New parameter for adaptive CR
                'CR_memory': [0.9],  # New parameter for CR memory
                'CR_memory_size': 5  # New parameter for CR memory size
            }
        else:
            self.params = params
        self.func = func
        ...
        self.CR_memory = self.params['CR_memory']
    ...
    def mutation(self, pop, idx, F, best):
        if self.params['mutation_strategy'] == 'current-to-best/2/bin':
            ...
            mutant = pop[idx] + F * (best - pop[idx] + b - c + d - a)
        else:  # Fallback to a simpler strategy if needed
            idxs = [i for i in range(len(pop)) if i != idx]
    ...
    def run(self):
        ...
        while self.evals < self.max_evals:
            ...
            for i in range(pop_size):
                F = self.adaptive_F()
                CR = self.adaptive_CR() if self.params['adaptive_CR'] else self.params['CR']
                mutant = self.mutation(pop, i, F, best)
