...
class Algorithm():
    def __init__(self, func, dim, bounds, max_evals, params=None):
        if params is None:
            self.params = {
                'pop_size': 200,  # Further increased population size
                'F_base': 0.5,  # Base value for F
                'F_amp': 0.3,  # Adjusted amplitude for F variation
                'CR_base': 0.9,  # Base value for CR
                'CR_amp': 0.05,  # Amplitude for CR variation
                'p': 0.25,  # Increased probability for choosing best individuals
                'archive_size': 250,  # Further increased size of the archive
                'mutation_strategy': 'best/2/bin',  # New mutation strategy
                'adaptive': True,  # Enable adaptive parameters
                'F_adapt': True,  # Enable adaptive F
                'CR_adapt': True,  # Enable adaptive CR
                'memory': True,  # Enable memory for F and CR
            }
        else:
            self.params = params
        self.func = func
