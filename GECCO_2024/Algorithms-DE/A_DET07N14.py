...
class Algorithm():
    ...
    def __init__(self, func, dim, bounds, max_evals, params=None):
        if params is None:
            self.params = {
                'pop_size': 140, 'F_base': 0.6, 'CR_base': 0.85, 'p': 0.25, 'beta': 0.45,
                'archive_size': 140, 'mutation_strategy': 'current-to-pbest/1/bin',
                'adaptive': True, 'local_search_prob': 0.03, 'local_search_step': 0.04,
                'elite_size': 15, 'elite_local_search_prob': 0.15, 'memory': True,
                'memory_size': 15, 'diversification_prob': 0.15, 'landscape_analysis_interval': 15,
                'F_adapt_rate': 0.07, 'CR_adapt_rate': 0.12, 'adaptive_diversification': True,
                'exploration_phase': False, 'F_dynamic': True, 'CR_dynamic': True,
                'hybrid_local_search': True, 'hybrid_ls_prob': 0.05, 'adaptive_ls_step': True
            }
        else:
            self.params = params
        self.func = func
    ...
    def adapt_parameters(self, ruggedness):
        if ruggedness > 0.1:
            ...
            if self.params['adaptive_ls_step']:
                self.params['local_search_step'] *= 1.1
        else:
            self.params['F_base'] -= self.params['F_adapt_rate']
            self.params['CR_base'] += self.params['CR_adapt_rate']
            if self.params['adaptive_diversification']:
            ...
            if self.params['adaptive_ls_step']:
                self.params['local_search_step'] *= 0.9
        ...
        self.params['local_search_step'] = np.clip(self.params['local_search_step'], 0.01, 0.1)

    def hybrid_local_search(self, best):
        if np.random.rand() < self.params['hybrid_ls_prob']:
            return self.local_search(best, self.params['local_search_step'] * 2)
        return best, self.func(best)

    def run(self):
        pop_size = self.params.get('pop_size', 140)
        elite_size = self.params.get('elite_size', 15)
        ...
        while self.evals < self.max_evals:
            ...
            best, best_fitness = self.hybrid_local_search(pop[best_index])
