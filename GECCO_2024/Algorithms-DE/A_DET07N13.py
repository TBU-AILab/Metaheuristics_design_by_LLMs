...
class Algorithm():
    ...
    def __init__(self, func, dim, bounds, max_evals, params=None):
        if params is None:
            self.params = {
                'pop_size': 130, 'F_base': 0.6, 'CR_base': 0.8, 'p': 0.25, 'beta': 0.45,
                'archive_size': 130, 'mutation_strategy': 'current-to-pbest/1/bin',
                ...
                'hybrid_local_search': True, 'hybrid_ls_prob': 0.05
            }
        else:
            self.params = params
        self.func = func
    ...
    def run(self):
        pop_size = self.params.get('pop_size', 130)
