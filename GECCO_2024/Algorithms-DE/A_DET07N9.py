...
class Algorithm():
    ...
    def __init__(self, func, dim, bounds, max_evals, params=None):
        if params is None:
            self.params = {
                'pop_size': 100, 'F': 0.8, 'CR': 0.9, 'p': 0.15, 'beta': 0.4,
                ...
                'memory_size': 10, 'diversification_prob': 0.1, 'landscape_analysis_interval': 25,
                'F_adapt': 0.05, 'CR_adapt': 0.1
            }
        else:
            self.params = params
        self.func = func
    ...
    def adapt_parameters(self, ruggedness):
        if ruggedness > 0.1:
            self.params['F'] += self.params['F_adapt']
            self.params['CR'] -= self.params['CR_adapt']
        else:
            self.params['F'] -= self.params['F_adapt']
            self.params['CR'] += self.params['CR_adapt']
