...
class Algorithm():
    def __init__(self, func, dim, bounds, max_evals, params=None):
        if params is None:
            self.params = {
                ...
                'FLA_step': 0.1,  # New parameter for Firefly Algorithm step size
                'FLA_absorption': 1.0  # New parameter for Firefly Algorithm light absorption coefficient
            }
        else:
            self.params = params
        self.func = func
    ...
    def firefly_movement(self, firefly_i, firefly_j, attractiveness):
        # Move firefly_i towards firefly_j based on their relative attractiveness
    ...
    def run(self):
        ...
        while self.evals < self.max_evals:
            ...
            for i in range(pop_size):
                ...
                # Firefly Algorithm movement for additional exploration
                for j in range(pop_size):
                    if fitness[j] < fitness[i]:
                        attractiveness = 1 / (1 + self.params['FLA_absorption'] * np.linalg.norm(pop[i] - pop[j]))
                        pop[i] = self.firefly_movement(pop[i], pop[j], attractiveness)
