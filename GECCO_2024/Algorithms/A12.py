...
class Algorithm():
    def __init__(self, func, dim, bounds, max_evals, params=None):
        if params is None:
            self.params = {
                "pop_size": 400,  # Increased population size for broader initial search
                "step_size": 0.005,  # Further reduced step size for precision
                "prt": 0.85,  # Increased perturbation for extensive exploration
                "path_length": 6.0,  # Extended path length for deeper search
                "mutation_rate": 0.3,  # Increased mutation rate for diversity
                "crossover_rate": 1.0,  # Ensuring crossover happens every time
                "elite_rate": 0.4,  # Increased elitism rate to preserve top solutions
                "diversification_rate": 0.1,  # Increased diversification rate for escaping local optima
            }
        else:
            self.params = params
        self.func = func
