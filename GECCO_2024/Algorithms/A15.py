...
class Algorithm():
    def __init__(self, func, dim, bounds, max_evals, params=None):
        if params is None:
            self.params = {
                "pop_size": 300,  # Further increased population size for broader initial search
                ...
                "mutation_rate": 0.18,  # Slightly increased mutation rate
                ...
                "local_search_rate": 0.15,  # Increased local search rate for more frequent refinement
        ...
        while evals < self.max_evals:
            ...
            for i in range(elite_size, pop_size):
                ...
                if np.random.rand() < local_search_rate:
                    new_pop[i], local_evals = self.local_search(new_pop[i])
                    new_pop_cf[i] = self.func(new_pop[i])
                    evals += local_evals
